"""``mnem.api.doctor`` - pure-function doctor report.

Delegates to ``mnem.commands.doctor.build_report()`` which fans out to
...
def doctor() -> tuple[dict, int]:
    ...
    The dict shape is identical to what ``mnem doctor --json`` writes to
    stdout.  The exit_code follows CONVENTIONS.md (0 ok, 1 user-fixable,
    2 transient, 3 auth).

    No stdout/stderr is written.
    """
    from mnem.commands.doctor import build_report
    return build_report()
