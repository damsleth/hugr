"""Shared helper: call ``mnem.commands.passthrough.run()`` and return bytes.
...
from mnem.config import yaams_config_env_for_args
from mnem.failure import parse_stdout, write_error_log
...
def call(
    ...
    *,
    ...
    if mapping.interactive:
        raise ValueError(
            f"interactive verb cannot be called via mnem.api: "
            f"{' '.join(verb_args)}"
        )
