"""Fused high-level verbs for the mnem suite.
...
def ask(question: str, *, k: int = 10, live: bool = True) -> dict[str, Any]:
    ...
    if live:
        _append_source(
            ...
            source="owa-cal",
            command="events",
            verb_args=["calendar", "events", "--search", question],
        )
        _append_source(
            sources,
            warnings,
...
def find(kind: str, query: str, *, k: int = 10) -> dict[str, Any]:
    ...
    routes: dict[str, tuple[str, str, list[str]]] = {
        ...
        "event": ("owa-cal", "events", ["calendar", "events", "--search", query]),
...
def inbox() -> dict[str, Any]:
    ...
    for source, command, verb_args in (
        ...
        ("owa-cal", "events", ["calendar", "events", "--today"]),
    ...
    *,
    ...
    return {
        ...
        "error": None if ok else {
            ...
            "hint": "Run `mnem ledger notes --help` to inspect the ledger note commands.",
        },
    }
