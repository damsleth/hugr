"""Async stdio MCP server entry point for ``mnem mcp --stdio``.
...
at startup from ``mnem.api`` signatures via ``mnem.mcp.tools``.
"""
...
from mnem.mcp._adapter import adapt
from mnem.mcp.tools import _reverse_name_map, build_tool_defs
...
async def run() -> None:
  ...
  async def call_tool(name: str, arguments: dict) -> CallToolResult:
    ...
    if fn_name is None:
      # Try stripping the "mnem." prefix and replacing dots with underscores.
      fn_name = name.removeprefix("mnem.").replace(".", "_")
