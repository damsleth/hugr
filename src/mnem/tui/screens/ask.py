"""mnem.tui.screens.ask - the Ask screen.

Layout:
  - Header: app title + doctor status
  - Nav bar: [a]sk [f]ind [i]nbox [r]emember [d]octor [q]uit
  - Input: query field
  - Results: ranked result list (raw text from mnem.api.yaams_query)

TODO (01.2): replace yaams_query call with mnem.api.ask() once that
...
class AskScreen(Screen):
  ...
  Provides a query input and a result pane.  Calls mnem.api.yaams_query
  ...
  def _run_query(self, query: str) -> None:
    """Call mnem.api.yaams_query and render the raw result.

    TODO (01.2): swap for mnem.api.ask(query) once plan 01.2 ships.
