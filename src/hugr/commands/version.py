"""``hugr version`` - hugr's own version + observed component versions.
...
``hugr._minimums.PACKAGES`` so there is exactly one source of truth.
"""
...
def build_report() -> dict:
  ...
  Called by ``hugr.api.version`` so surfaces never shell out just for
  version info.
  """
  return _data_doc()
