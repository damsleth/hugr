"""Generic passthrough handler for hugr verbs that route to underlying tools.

Used by `hugr query` and `hugr ingest`. The router translates the
hugr-side argv to the underlying-tool argv; this module:
...
4. On failure: emits a redacted one-line summary on stderr,
   appends the captured stderr to ~/.local/state/hugr/last-error.log
   through hugr's own redactor, and exits with the underlying
   tool's exit code.
"""
...
def _run_interactive(argv: Sequence[str]) -> int:
  ...
  try:
    ...
    sys.stderr.write(f"x hugr: binary not on PATH: {exc}\n")
    return 127
...
def run(
  ...
  *,
  ...
  """Dispatch ``hugr <verb-args>`` through the translation table.

  ``top_level_json`` is the value of ``hugr --json <verb>`` and
  propagates as intent, not as a flag rewrite. Per CONVENTIONS.md
  interactive commands reject --json; we enforce that at the hugr
  layer rather than letting the child fail mysteriously.
  """
  resolved = lookup(verb_args)
  if resolved is None:
    sys.stderr.write(
      f"x hugr: no such verb: {' '.join(verb_args) or '<empty>'}\n"
      f"    Try: hugr hello\n"
    )
    return 1
  ...
  if mapping.interactive:
    if top_level_json:
      ...
      sys.stderr.write(
        f"x hugr: interactive command rejects --json: {verb_str}\n"
