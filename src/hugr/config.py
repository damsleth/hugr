"""Master config for hugr: ``$XDG_CONFIG_HOME/hugr/config.toml``.

Schema (flat TOML at root):
...
Parsing uses ``tomllib`` (stdlib, 3.11+). The shape is flat top-level
key/value scalars.
"""
...
def read_master(path: Path | None = None) -> dict[str, str]:
  """Parse the master config (flat TOML) into a str->str dict.

  Unknown top-level keys are preserved so future fields don't trip the
  parser. Non-scalar values (tables, arrays) are ignored. Returns
...
def render_master(
  *,
  ...
  """Hand-roll the master config file body as TOML."""
