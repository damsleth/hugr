"""Graceful-failure middleware for the hugr router.

Implements the subprocess capture contract from
hugr/CONVENTIONS.md:

- hugr invokes underlying CLIs with --json (machine mode).
- stdout is captured for envelope parsing.
- stderr is captured, run through hugr's own redaction pass, then
  appended to ~/.local/state/hugr/last-error.log.
...
def _state_dir() -> Path:
  """``~/.local/state/hugr`` with the standard mode-0700 perms."""
...
def run_subprocess(
  ...
  *,
  ...
  to argv if not already present. hugr always invokes JSON-capable
...
def one_line_summary(*, tool: str, result: SubprocessResult) -> str:
  """Build the redacted one-line stderr message hugr shows on failure."""
  log = log_path()
  if result.crashed:
