"""hugr.tui.screens.inbox - cross-tool inbox view."""
...
from hugr.tui.screens._render import render_inbox_doc
...
class InboxScreen(Screen):
  """Cross-tool inbox: mail + calendar + ledger loops + promote list."""
  ...
  def compose(self) -> ComposeResult:
    ...
    yield Static("Loading inbox...", id="results")
    yield Footer()
  ...
  def _render_doc(self, doc: dict) -> None:
    self.query_one("#results", Static).update(render_inbox_doc(doc))

  def _render_error(self, detail: str) -> None:
    self.query_one("#results", Static).update(f"[red]Inbox failed[/red]\n{detail}")
