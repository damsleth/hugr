...
When the user has no hugr config yet, the verbs that touch the
yaams DB exit 4 with a clean ``Run: hugr init`` pointer instead of
...
from hugr.cli import _VERBS_NEEDING_CONFIG, _ensure_config, cli


def test_verbs_needing_config_includes_phase3a_verbs():
  assert ("recall",) in _VERBS_NEEDING_CONFIG
...
def test_ensure_config_bypasses_when_user_passes_explicit_config(monkeypatch, tmp_path: Path):
  """`hugr query --config /elsewhere/cfg.yaml` should NOT trigger the
...
def test_ensure_config_bypasses_when_yaams_config_env_set(monkeypatch, tmp_path: Path):
  ...
  make `yaams query` and `hugr query` resolve to the same config.
  hugr's first-run guard must defer to the env var the same way
  yaams does."""
  monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
...
def test_ensure_config_bypasses_when_hugr_config_env_set(monkeypatch, tmp_path: Path):
  """HUGR_CONFIG is the suite-wide override; it should still bypass."""
  monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
  monkeypatch.delenv("YAAMS_CONFIG", raising=False)
  monkeypatch.setenv("HUGR_CONFIG", "/elsewhere/cfg.yaml")
  assert _ensure_config(("query",)) is None
...
def test_auth_status_does_not_need_config(monkeypatch, tmp_path: Path):
  ...
  hugr should NOT block it on the missing hugr config."""
  monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
  ...
  # intercept.
  assert result.exit_code != 4 or "hugr init" not in (result.stderr_bytes or b"").decode("utf-8", "ignore")
