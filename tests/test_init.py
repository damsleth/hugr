...
from hugr.commands.init import _build_yaams_config
from hugr.config import master_config_path, read_master, render_master
...
def test_init_quick_json_writes_master_and_yaams_config(monkeypatch, tmp_path: Path):
  ...
  assert "hugr recall" in doc["next_steps"][0]
...
def test_build_yaams_config_round_trips_through_yaml_parsing():
  ...
  try:
    ...
    return  # PyYAML isn't a hard dep of hugr; skip if it's absent.
...
def test_init_reuses_existing_yaams_config_in_place(monkeypatch, tmp_path: Path):
  """If `~/.config/yaams/config.yaml` already exists, hugr records
